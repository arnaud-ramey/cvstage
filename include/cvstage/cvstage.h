...
public:
  ...
  void draw_axes(const int thickness = 2) {
    draw_arrow(_viz, world2pixel(0, 0), world2pixel(1, 0),
               CV_RGB(255, 0, 0), thickness);
    draw_arrow(_viz, world2pixel(0, 0), world2pixel(0, 1),
  ...
  inline void set_visible_window(const double xmin, const double xmax,
    ...
    //ROS_INFO("set_visible_window(x:%g->%g, y:%g->%g)\n", xmin, xmax, ymin, ymax);
  ...
  inline bool mouse_move_callback(int event, int x, int y) {
    // ROS_INFO("mouse_move_callback(event:%i, x:%i, y:%i)\n", event, x, y);
    ...
    // use middle button for zoom/unzoom
