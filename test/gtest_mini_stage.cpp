...
TEST(TestSuite, reproject_points) {
  ...
  double origin_dist = cvstage_plugins::distance_points(origin, origin_back_to_world);
  ...
  for (unsigned int i = 0; i < 50; ++i) {
    ...
    ASSERT_NEAR(fabs(dist), 0, 0.1)
        << "test_pt:" << cvstage_plugins::printP2(test_pt).c_str()
        << ", to pixel:" << cvstage_plugins::printP2(ms.world2pixel(test_pt)).c_str()
        << ", back_to_world:" << cvstage_plugins::printP2(back_to_world).c_str()
        << ", dist:" << dist;
  } // end loop i}
}
...
TEST(TestSuite, moving_origin_and_heading) {
  ...
  int t;
  for (int i = 0; i < 100; ++i) {
    ms.set_origin(cv::Point2f(cos(t), sin(t)));
    t++;
    ms.clear();
    ms.draw_grid(.5f, 150);
    ms.draw_axes();
...
TEST(TestSuite, reproject_image) {
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    ms.draw_grid(.5f, 150);
    ms.draw_axes();
    cvstage_plugins::reproject_image(ms, rgb, depth, depth_camera_model,
